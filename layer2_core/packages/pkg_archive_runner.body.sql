  ...
  */
  ...
  BEGIN
    ...
    IF l_stop_step = 'DISCOVER' THEN
      PKG_ARCHIVE_LOG.prc_log_message(p_run_id => l_run_id, p_log_msg => 'Runner finished at stop step DISCOVER');
    ...
    IF l_stop_step = 'ARCHIVE' THEN
      PKG_ARCHIVE_LOG.prc_log_message(p_run_id => l_run_id, p_log_msg => 'Runner finished at stop step ARCHIVE');
      PKG_ARCHIVE_LOG.prc_finish_run(l_run_id, 'SUCCESS');
      RETURN;
    END IF;
    ...
    IF l_stop_step = 'QUALITY' THEN
      PKG_ARCHIVE_LOG.prc_log_message(p_run_id => l_run_id, p_log_msg => 'Runner finished at stop step QUALITY (TRUNCATE skipped)');
      PKG_ARCHIVE_LOG.prc_finish_run(l_run_id, 'SUCCESS');
      RETURN;
    END IF;
    ...
    PKG_ARCHIVE_LOG.prc_log_message(p_run_id => l_run_id, p_log_msg => 'Runner finished all steps');
    PKG_ARCHIVE_LOG.prc_finish_run(l_run_id, 'SUCCESS');
  EXCEPTION
    WHEN OTHERS THEN
