CREATE OR REPLACE VIEW TW_ARCHIVE_SOURCE_PARTITIONS_VW
AS
WITH cfg AS (
    ...
    FROM cfg c,
         XMLTABLE(
           ...
           PASSING DBMS_XMLGEN.GETXMLTYPE(
             ...
             '.ARCHIVE_PARTITION_INFO_VW' ||
             CASE
               WHEN c.source_db_link IS NOT NULL
                    AND UPPER(TRIM(c.source_db_link)) NOT IN ('LOCAL', 'NONE')
                 THEN '@' || DBMS_ASSERT.SIMPLE_SQL_NAME(c.source_db_link)
             END ||
