CREATE OR REPLACE EDITIONABLE PACKAGE BODY PKG_REPLICA_TL_LOGGING AS

  /*
    Package      : PKG_REPLICA_TL_LOGGING
    Developer    : Tomasz Lesinski
    Date         : 2020-02-24
    Purpose      : Logging package

    Prerequisite : TBL_REPLICA_PROCESS_LOG, REPLICA_PROCESS_LOG_SEQ
...
FUNCTION fn_generate_json_log
(
  p_log_msg	  IN CLOB,
  p_log_type	  IN VARCHAR2,
  p_log_date	  IN VARCHAR2
)
RETURN CLOB
IS
  l_json_entry CLOB;
...
BEGIN
  ...
  l_msg_json := fn_generate_json_log(p_log_msg => p_log_msg, p_log_date => l_sysdate, p_log_type => p_log_type);
  ...
  IF p_log_lvl = 'N' OR ( p_log_lvl = 'D' AND g_log_lvl = 'D' ) THEN
	...
	SET
	  ...
	  a.log_json	  = rtrim(a.log_json,']') || to_clob(',') || b.log_json ||to_clob(']'),
	...
	USING
	  ...
	  ,p_last_err_code
	  ,p_last_err_desc
	  ,PKG_REPLICA_TL_LOGGING.g_sttus_undefined_const;
...
-- dumps oracle stack if error occures into table MD_PRCSS_LOG
  ...
  BEGIN
    ...
    (
      ...
      p_log_id	       => p_log_id,
      p_log_sttus      => PKG_REPLICA_TL_LOGGING.g_sttus_error_const,
      p_end_date       => SYSDATE,
      p_log_msg        => l_message,
...
-- and local log sequence REPLICA_PROCESS_LOG_SEQ
...
IF l_check = 0 THEN
  ...
  l_sql := 'CREATE SEQUENCE REPLICA_PROCESS_LOG_SEQ START WITH 1 INCREMENT BY 1';
